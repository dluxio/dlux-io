...
function buyFTHive(set, uid, price, token){
    ...
    broadcastTransfer({to:'dlux-cc',[`${token.toLowerCase()}`]:price , memo: `NFT ${set}:${uid}`}, `Buying a ${set} mint token`)
}
function sellDEX(dlux, hive, hbd, hours, callback){
 ...
 function buyDEX(hive, hbd, dlux, hours, callback){
    ...
    if (dlux){
     ...
     broadcastTransfer({ to: 'dlux-cc', hive, hbd, memo:JSON.stringify({rate, hours})}, "dlux_dex_sell", `Buying DLUX with ${parseFloat((hive||hbd)/1000).toFixed(3)} ${hive?'HIVE':'HBD'} ${andthen}`)
 }
