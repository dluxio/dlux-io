...
<main role="main" class="flex-shrink-0">
  ...
  <div class="card-columns p-3" id="dluxCards" is="dmx-repeat" dmx-bind:repeat="dluxList.data.result">
    ...
    <div class="card text-white bg-dark mt-2 mb-3">
      ...
      <a href="#detailModal" class="a-1" data-toggle="modal" dmx-on:click="data_detail.select(url)" dmx-bind:onclick="window.history.pushState('{{url}}','{{title}}', '/blog/@{{author}}/{{permlink}}');updateModalData('{{dluxLoad.data.result.created}}', '{{dluxLoad.data.result.json_metadata.toString()}}', '{{dluxLoad.data.result.title}}', '{{dluxLoad.data.result.body}}', '{{dluxLoad.data.result.active_votes.toString()}}', '{{dluxLoad.data.result.children.toString()}}', '{{dluxLoad.data.result.total_payout_value}}')">
