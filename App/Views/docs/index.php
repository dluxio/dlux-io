...
<article class="docs-article" id="whitepaper">
    ...
    <p>It details the philosophy and design principles behind the dlux-io platform. Back then, HIVE was called HIVE.</p>
...
<article class="docs-article" id="token">
    ...
    <p>dlux tokens allow us to reward user actions like reblogging our ads, sharing dlux on other social platforms, and other types of engagement.</p>
    <ul>
        ...
        <li>dlux maintains a state node to generate the reward pool</li>
    ...
    <p>The reward pool is used to incentivize others to run state nodes to verify integrity and that we're running our code fairly and correctly, provable through the hash. The 20 fastest verification responses earn rewards.</p>
</article>
...
<article class="docs-article" id="nodes">
    ...
    <p>The dlux token architecture is unique in that it uses web worker nodes listening to every signed Hive Transaction (HTx) via HIVE (GitHub Source) to determine state. The can also perform actions on those signed HTx because the nodes have Hive Credentials (HC), which they can use to place the transaction back into the HIVE transaction stream.</p>
    ...
    <ul>dlux nodes, are super easy to run, and can do things like:
