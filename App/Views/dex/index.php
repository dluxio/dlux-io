...
<dmx-data-view id="auctionsToken" dmx-bind:data="auctions.data.result" sorton="time" pagesize="10"></dmx-data-view>
	   ...
	   <div class="card-columns my-3" id="sales-token-cards">
		<div class="card text-white bg-dark" dmx-repeat:repeatsalestoken1="salesToken.data">
		 <a href="#inventoryModal" class="a-1" data-toggle="modal" dmx-on:click="inventory_iterator.select($index);inventory_detail.select(uid)">
		  <div class="card-header text-center" style="background: purple"><h5 class="card-title">{{set}} <span style="color: rose">{{uid}}</span></h5></div>
		 ...
		 <div class="card-footer text-center">Buy Now: {{price.nai()}}</div>
		</a></div>
		 ...
		 <a href="#inventoryModal" class="a-1" data-toggle="modal" dmx-on:click="inventory_iterator.select($index);inventory_detail.select(uid)">
		  ...
		  <div class="card-body"><p class="card-text">Behold&#33; The DLUX Founders Token. Own a piece of dlux in the form of an NFT that is redeemable for...</p></div>
		 <div class="text-center " style="background: crimson"><h5 dmx-bind:id="timer-{{set}}-{{uid}}" class="mb-0">{{time.animateTime(set, uid)}}</h5></div>
		 <div class="card-footer text-center">Auction Bid: {{price.nai()}}</div>
		</a></div>
	  			...
	  			<div style="border-bottom: #FFFFFF thick solid"><h1 class="text-white p-0 m-0">Unminted</h1></div>
	   			<div class="card-columns my-3" id="sales-mint-cards" >
					...
					<div class="card text-white bg-dark" dmx-repeat:repeatsalesmint2="$value">
					<a href="#inventoryModal" class="a-1" data-toggle="modal" dmx-on:click="inventory_iterator.select($index);inventory_detail.select(uid)">
						...
						<div class="card-footer text-center"><div dmx-repeat:repeatsalesmint3="items">{{price.nai()}}</div></div>
					</div>
				</div>
			</div>
